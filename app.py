...
def detect():
    ...
    if request.method == "POST":
        try:
            ...
            if use_uploaded:
                ...
                if file_ext not in allowed_extensions:
                    ...
                    return render_template("detect.html",
                                           error="Upload failed. The uploaded file must be a JPG or PNG image."), 400
                ...
                if file_ext not in allowed_extensions:
                    ...
                    return render_template("detect.html",
                                           error="Upload failed. Please upload only JPG or PNG images."), 400
            ...
            logger.debug(f"Starting segmentation process for image: {image_path}")
            try:
                ...
                return render_template("detect.html", 
                                     error=f"Image processing failed during segmentation: {e}"), 500

            # Check if segmentation produced any outputs
            if not os.path.exists(SEGMENT_OUTPUT_DIR) or not any(os.listdir(SEGMENT_OUTPUT_DIR)):
                logger.warning("No components detected in the image")
                return render_template("detect.html", 
                                     error="No network components were detected in this image. Please try a different image with visible network equipment."), 400
            ...
            try:
                ...
                return render_template("detect.html", 
                                     error=f"Processing failed during embeddings generation: {e}"), 500
            ...
            if not os.path.exists(COMPARE_OUTPUT_CSV):
                ...
                return render_template("detect.html", 
                                     error="Processing completed but results file was not generated"), 500
...
def csv_to_json(csv_path, json_path):
    """Convert CSV to JSON with grouping by cropped_image and filtering for similarity score > 0.7"""
    try:
        logger.debug(f"Converting {csv_path} to JSON at {json_path}")
        if not os.path.exists(csv_path):
            logger.error(f"CSV file not found: {csv_path}")
            return False

        # Read the CSV with pandas for easier filtering
        ...
        # If no data after filtering, return empty result
        if df.empty:
            logger.warning(
                "No data in CSV after filtering for similarity_score > 0.7")
            with open(json_path, "w", encoding="utf-8") as f:
                json.dump([], f, indent=4)
            return True

        # Find the grouping key for organizing data
        grouping_key = None
        if 'cropped_image' in df.columns:
            grouping_key = 'cropped_image'
        elif 'cropped image' in df.columns:
            grouping_key = 'cropped image'

        # If no grouping key found, we can't continue
        if not grouping_key:
            logger.error("No valid grouping key (cropped_image) found in CSV")
            return False

        # Convert DataFrame to dictionary for processing
        rows = df.to_dict('records')

        # Group by the grouping_key
        grouped_data = {}
        for row in rows:
            # Use the image path as our label
            label = row.get(grouping_key)
            if label:
                # Extract a shorter label from the path
                short_label = label.split('/')[-1].replace('.png', '').replace(
                    '.jpg', '')

                if short_label not in grouped_data:
                    grouped_data[short_label] = []

                # Include only relevant fields
                filtered_row = {}
                # Include fields we want to keep
                for field in [
                        'category', 'name', 'description', 'similarity_score'
                ]:
                    if field in row:
                        filtered_row[field] = row[field]

                # Add image path explicitly
                filtered_row['image_path'] = label

                grouped_data[short_label].append(filtered_row)

        # Format into the final structure
        result = []
        for label, matches in grouped_data.items():
            result.append({
                "cropped_label": label,  # Use the extracted label
                "category":
                matches[0].get('category',
                               'Unknown'),  # Include category at top level
                "matches": matches
            })
        ...
        logger.debug(
            f"Successfully converted CSV to JSON with {len(result)} groups")
        return True
